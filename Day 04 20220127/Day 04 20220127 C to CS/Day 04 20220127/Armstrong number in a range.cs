...
class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            int F_Num,T_Num,Loop_Index;
            
            // Accepting user input or colleting base data
            Console.WriteLine("Enter Starting Number to List Armstrong Numbers ");
            F_Num = Convert.ToInt32(Console.ReadLine());
            Console.WriteLine("Enter Ending Number of List ");
            T_Num = Convert.ToInt32(Console.ReadLine());

            // Check & Displaying output
            Console.WriteLine("List of Armstrong number from " + F_Num + " to " + T_Num);
            for(Loop_Index=F_Num;Loop_Index<=T_Num;Loop_Index++)
            if (Armstrong(Loop_Index))
